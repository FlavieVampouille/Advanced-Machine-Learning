...
class Agent:
    ...
    def reset(self):
        ...
        self.current_action = Action(np.random.randint(1,len(Action)+1))
    ...
    def getAction(self):
        ...
        # exploitation of knowledge
            ...
            self.current_action = Action(1 + np.argmax(self.Q[str(self.getState())]))
    ...
    def nextState(self,s,reward):
        ...
        if self.first_visit == False :
            self.futureState = s
            self.Q[str(self.getState())][self.current_action-1] += self.alpha * ( reward + self.gamma * max(self.Q[str(self.futureState)] - self.Q[str(self.getState())][self.current_action-1]) )
            self.visit[str(self.getState())] += 1
        else :
