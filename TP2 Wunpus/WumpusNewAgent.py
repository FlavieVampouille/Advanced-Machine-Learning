...
# prudent agent who uses the flash when he smells the wumpus 
# and moves after in the same direction  
class Agent:
    def __init__(self):
        self.reset()
        self.rd = -1
    ...
    def getAction(self):
        x, y, smell, breeze, numFlash = self.state_
       
        # si risque de rencontrer le wumpus
        if smell != 0 :
            # utilise le flash
            if self.rd == -1 and numFlash != 0 :
                self.rd = np.random.randint(1,5)
                action = Action(4+self.rd)
            # si plus de flash, avance au hasard
            elif self.rd == -1 and numFlash == 0 :
                action = Action(np.random.randint(1,5))
            # après avoir utiliser le flash avance dans la même direction
            else :
                action = Action(self.rd)
                self.rd = -1    
        
        # si pas de wumpus à proximité ou numFlash nul
        else:
            action = Action(np.random.randint(1,5))
