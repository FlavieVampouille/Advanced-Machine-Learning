...
class Agent:
        ...
        for i in xrange(4):
            for j in xrange(4):
                for k in xrange(2):
                    for l in xrange(2):
                        for h in xrange(0,6):
                            self.Q[str([i, j, k, l, h])] = np.zeros(len(Action))
                            self.visit[str([i, j, k, l, h])] = np.zeros(len(Action))
        self.qstate = np.zeros(len(Action))
        
    def reset(self):
        self.first_visit = True
        self.current_action = Action(np.random.randint(1,5))
        
    def getAction(self):
        self.qstate = [UCBValue(c,n,self.count) for c, n in zip(self.Q[str(self.getState())], self.visit[str(self.getState())])]
        self.current_action = Action(1 + np.argmax(self.qstate))
    ...
    def nextState(self,s,reward):
        ...
        if self.first_visit == False :
            self.Q[str(self.getState())][self.current_action-1] = (self.Q[str(self.getState())][self.current_action-1] * self.visit[str(self.getState())][self.current_action-1] + reward ) / self.visit[str(self.getState())][self.current_action-1]
            self.visit[str(self.getState())][self.current_action-1] += 1
            self.count += 1
        else :
